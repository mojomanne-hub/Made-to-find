...
export default async function EditLocationPage({ params }: Props) {
  ...
  const [{ data: location }, { data: shelves }] = await Promise.all([
    supabase.from("locations").select("*").eq("id", id).is("deleted_at", null).maybeSingle(),
    supabase.from("shelves").select("id, name").eq("location_id", id).order("position"),
  ]);
