...
function DynIcon({ name, className }: { name: string | null; className?: string }) {
  ...
  if (isEmoji) return <span style={{ fontSize: "4rem", lineHeight: 1 }}>{name}</span>;
  ...
  return (
    <>
      ...
      <div className="rounded-2xl overflow-hidden mb-5 w-full"
        ...
        {item.image_url ? (
          <img src={item.image_url} alt={item.name} className="w-full rounded-2xl object-contain" />
        ) : item.icon ? (
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ minHeight: "280px" }}>
            <LucideIcons.Box className="h-20 w-20 text-white/70" />
          </div>
        )}
      </div>
