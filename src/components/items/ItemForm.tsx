...
interface ItemFormProps {
  item?:                  Item & { icon?: string | null; image_url?: string | null };
...
export function ItemForm({ item, locations, preselectedLocationId, userId, groupId }: ItemFormProps) {
  ...
  const [color,       setColor]       = useState((item as (typeof item & { color?: string | null }) | undefined)?.color ?? LOCATION_COLORS[0].value);
    ...
    try {
      ...
      if (isEditing) {
        ...
        const { data, error } = await supabase
          .from("items").insert({ ...payload, user_id: user.id, ...(groupId ? { group_id: groupId } : {}) }).select().single();
        if (error) { setServerError(`Fehler: ${error.message}`); return; }
